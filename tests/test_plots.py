...
def test_miss_bar_chart_returns_empty_when_no_missing(tmp_path):
    ...
    out = miss_bar_chart(missing_df, out_dir=tmp_path)
    assert out == ""


def test_miss_bar_chart_creates_file_when_missing_exists(tmp_path):
    missing_df = pd.DataFrame(
        {
            "column": ["a", "b"],
            "missing_count": [1, 3],
...
def test_hist_plots_returns_empty_when_no_numeric_cols(tmp_path):
    df = pd.DataFrame(
        {
            "name": ["a", "b", "c"],
            "city": ["x", "y", "z"],
    ...
    paths = hist_plots(df, out_dir=tmp_path)
    assert paths == []


def test_hist_plots_creates_up_to_three_files(tmp_path):
    df = pd.DataFrame(
        {
