...
def hist_plots(df) -> list[str]:
    ...
    columns_filtered = (   
        unique_ratio_per_col[
            (unique_count_per_col>=10) &
            (unique_ratio_per_col>=0.01)
        ]
        .index
        .to_list()
        )
    ...
    for i in range(len(top_std)):
        ...
        df_numeric_filtered[col].dropna().plot(xlabel=col, ylabel="Frequency", grid=True, kind='hist', bins=30, ax=ax, title=f"Distribution: {col}")
        fig.tight_layout()
